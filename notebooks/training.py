...
def get_tiles(image: Image, tile_size: int):
    ...
    positions = [{'x_start': x, 'x_stop': x+tile_size, 'y_start': y, 'y_stop': y+tile_size} for x,y in itertools.product(offset_x, offset_y)]
    return positions
...
def get_document_tiles(pdf_directory, dpi=50):
    ...
    for pdf in tqdm(pdf_files):
        try:
            ...
            print('Unable to open', pdf, 'because', e)
            
    dummy_labels = np.zeros(len(pdf_tiles)).astype(int)
    dummy_labels[-1] = 1
    #return pdf_tiles
    return [(tile, label) for tile, label in zip(pdf_tiles, dummy_labels)]

def get_svg_signatures(svg_directory):
...
def get_signature_image(svg_spec: dict, width=100):
    img = Image.open(io.BytesIO(svg2png(url=str(svg_spec['path']), 
                                        output_width=100, 
                                        output_height=int(100*svg_spec['aspect_ratio']))))
    
    return img

def create_synthetic_image(doc_spec: dict, svg_specs: list, positive_prob: float=0.5, dpi: int=50) -> None:
    #print(doc_spec)
    ...
    if random.random() < positive_prob:
        width = int(random.gauss(mu=2*dpi, sigma=dpi/4))
...
def SyntheticImageBlock(svg_directory: Path, positive_prob: float = 0.5):
    ...
    return TransformBlock(type_tfms=lambda doc_spec: create_synthetic_image(doc_spec, svg_specs, positive_prob), batch_tfms=IntToFloatTensor)
...
get_label_from_x = GetLabelFromX()